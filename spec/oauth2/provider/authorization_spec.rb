...
describe OAuth2::Provider::Authorization do
  ...
  before do
    ...
    allow(OAuth2::Lib::SecureCodeScheme).to receive(:generate).and_return('s1', 's2', 's3')
  end
  ...
  describe "#grant_access!" do
    describe "when there is an existing authorization with no code" do
      ...
      it "generates and returns a code to the client" do
        ...
        @model.reload
        expect(@model.code).to_not be_empty
        expect(authorization.code).to_not be_empty
      end
    end
    ...
    describe "when there is an existing authorization with scopes" do
      before do
        ...
        params['scope'] = 'other_scope'
      end

      it "merges the new scopes with the existing ones" do
        authorization.grant_access!
        @model.reload
        expect(@model.scopes).to eq(Set.new(%w[foo bar other_scope]))
      end
    end
    ...
    describe "for token requests" do
      ...
      it "creates an Authorization in the database" do
        ...
        expect(authorization.client).to eq(@client)
        expect(authorization.code).to be_nil
        expect(authorization.access_token_hash).to eq(OAuth2::Lib::SecureCodeScheme.hashify("s1"))
        expect(authorization.refresh_token_hash).to eq(OAuth2::Lib::SecureCodeScheme.hashify("s2"))
      end
    end
    ...
    describe "for code_and_token requests" do
      ...
      it "creates an Authorization in the database" do
        ...
        expect(authorization.client).to eq(@client)
        expect(authorization.code).to eq("s1")
        expect(authorization.access_token_hash).to eq(OAuth2::Lib::SecureCodeScheme.hashify("s2"))
        expect(authorization.refresh_token_hash).to eq(OAuth2::Lib::SecureCodeScheme.hashify("s3"))
      end
    end
  end
